...
// Store the elements in a list
    ...
    private void sacuvajBSTCvorove(TreeNode koren, List<TreeNode> cvorovi) {
// Osnovni slučaj
        if (koren == null) {
            return;
        }
// Zapamti čvorove u infisknom poretku (sortirane)
        sacuvajBSTCvorove(koren.left, cvorovi);
        cvorovi.add(koren);
        sacuvajBSTCvorove(koren.right, cvorovi);
    }

    /* Rekurzivna funkcija za konstruisanje stabla */
...
// Funkcija konvertuje neuravnoteženo stablo u uravnotežno

    public TreeNode napraviBalansiranoStablo(TreeNode koren) {
// Sačuvaj čvorove datog BST u sortiranom poretku
        List<TreeNode> cvorovi = new ArrayList<>();
        sacuvajBSTCvorove(koren, cvorovi);
// Konstruiši BST iz liste cvoroviC
        return napraviStabloUtil(cvorovi, 0, cvorovi.size() - 1);
    }

    boolean jeBalansirano(TreeNode koren, VisinaStabla visina) {
//Ukoliko je stablo prazno, vrati true
        if (koren == null) {
            visina.visina = 0;
            return true;
        }
